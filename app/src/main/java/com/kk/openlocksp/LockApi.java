...
//发送数据
    public static boolean sendData(byte[] data, String[] rsMsg)
    {
        try
        {
            Received rs= mApp.COM1.getReceived(data);
...
//锁控板指令
    private static boolean lockCmd(byte cmdNo, byte boardNo, byte lockNo, String[] rsMsg)
    {
        try
        {
            ...
            Received rs= mApp.COM1.getReceived(sendData);

            if (rs!= null)
            {
                if (rs.getSize()==5 && rs.getmBuffe()[0]==cmdNo)
                {
                    if (cmdNo==(byte)0x80) //获取状态
                    {
